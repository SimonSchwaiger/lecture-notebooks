...
# install gmapping and local planner, because they for some reason are not installed by the automatic dependency install
RUN apt-get update && apt-get install -y --no-install-recommends ros-noetic-gmapping ros-noetic-dwa-local-planner
